...
class FrontendParameter
{
    ...
    , m_is_EQUALIZE( false )
    , m_is_SHOW_TRACK( 1 )
    , m_fastMaxThreshold( 70 )
    , m_fastMinThreshold( 30 )
    , m_grid_x( 3 )
    , m_grid_y( 3 )
    {
    }
    ...
    bool readFromYamlFile( const std::string& filename )
    {
        ...
        if ( !fs.isOpened( ) )
        {
            std::cerr << "ERROR: Wrong path to settings 'feature_config_file': " << filename
                      << std::endl;
            return false;
        }
        ...
        m_fastMaxThreshold = fs["fastMaxThreshold"];
        m_fastMinThreshold = fs["fastMinThreshold"];
        m_grid_x           = fs["m_grid_x"];
        m_grid_y           = fs["m_grid_y"];
        ...
        if ( m_is_SHOW_TRACK )
        {
            ...
            std::cout << "grid_x  " << m_grid_x << std::endl;
            std::cout << "grid_y  " << m_grid_y << std::endl;
        }
    ...
    FrontendType m_type;
    int m_fastMaxThreshold;
    int m_fastMinThreshold;
    int m_grid_x;
    int m_grid_y;
};
}
