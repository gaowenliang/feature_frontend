...
void
FeatureMonoTracker::rejectWithF( )
{
    if ( ptsPre( ).size( ) >= 8 )
    {
        ...
        std::vector< cv::Point2f > un_prev_pts( ptsPre( ).size( ) ), un_forw_pts( ptsThis( ).size( ) );
        for ( unsigned int i = 0; i < ptsPre( ).size( ); i++ )
        {
            ...
            //  ROS_DEBUG_STREAM( "(" << ptsPre( )[i].x << " " << ptsPre( )[i].y <<
            //  ")->(" //
            //                        << ptsThis( )[i].x
            //                        << " "
            //                        << ptsThis( )[i].y
            //                        << "), ("
            //                        << ptsPre( )[i].x - ptsThis( )[i].x
            //                        << ", "
            //                        << ptsPre( )[i].y - ptsThis( )[i].y
            //                        << ")" );
        }

        std::vector< uchar > status;
        cv::findFundamentalMat( un_prev_pts, un_forw_pts, cv::FM_RANSAC, F_Th( ), 0.99, status );
        int size_a = ptsPre( ).size( );
        reduce_vector( ptsPre( ), status );
        reduce_vector( ptsThis( ), status );
        reduce_vector( idsPts( ), status );
        reduce_vector( cntTrack( ), status );
...
void
FeatureMonoTracker::getPoints( std::vector< std::vector< Eigen::Vector3d > >& points, std::vector< int >& id )
{
