...
FeatureBase::readInPramFile( std::string camera_param_file, std::string feature_param_file )
{
    ...
    m_detector = new DetectorFast( m_param.m_min_dist,
                                   5, //
                                   m_param.m_fastMaxThreshold,
                                   m_param.m_fastMinThreshold );
...
void
FeatureBase::readInPramFile( std::string cam_file, std::string err_file, std::string feature_param_file )
{
    readCameraFromYamlFile( cam_file, err_file );
...
{
    ROS_DEBUG_STREAM( "addadd size " << m_pts_new.size( ) << " now size "
                                     << ( m_pts_new.size( ) + m_pts_pre.size( ) ) );
...
FeatureBase::detectNewFeatures( const cv::Mat& image )
{
    ...
    //    cv::goodFeaturesToTrack( image, m_pts_new, new_num, 0.1, m_param.m_min_dist,
    //    m_detector->setMask( m_pts_this ) );
...
std::vector< double >
FeatureBase::getPErrorAnglePoints( )
{
    return getErrorAngle( getPtsThis( ) );
}
...
std::vector< uchar >
FeatureBase::getTrackStatus( ) const
{
