...
{
    NUM_OF_CAM = ros_utils::readParam< int >( nh, "camera_num" );
    p_subImgs.resize( NUM_OF_CAM );
    ...
    for ( int camera_index = 0; camera_index < NUM_OF_CAM; camera_index++ )
    {
        ...
        = ros_utils::readParam< std::string >( nh, prefix + "image_topic" );

        p_subImgs[camera_index]
    ...
    ros_utils::App4ImgSynchronizer* sync
    = new ros_utils::App4ImgSynchronizer( ros_utils::AppSync4Images( 30 ),
                                          *( p_subImgs[0] ), //
                                          *( p_subImgs[1] ) );
                                ...
                                const sensor_msgs::ImageConstPtr& img_msg_r )
{
    ...
    ROS_DEBUG( "frontend all costs %f", t_track.toc( ) );
}
