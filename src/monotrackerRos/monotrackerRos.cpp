...
MonoTracker::readParameters( ros::NodeHandle nh )
{
    ...
    feature_files.push_back( ros_utils::readParam< std::string >( nh, "feature_config_file" ) );

    image_in_buf.resize( 1 );

    tracker = frontend::FrontendInit::newFrontend( )->init( frontend::MONO, cam_files, feature_files );
...
MonoTracker::image_callback( const sensor_msgs::ImageConstPtr& img_msg )
{
    ...
    if ( round( 1.0 * detect_count / ( img_msg->header.stamp.toSec( ) - first_image_time ) ) <= FREQ )
    {
        ...
        if ( abs( 1.0 * detect_count / ( img_msg->header.stamp.toSec( ) - first_image_time ) - FREQ ) < 0.01 * FREQ )
        {
            first_image_time = img_msg->header.stamp.toSec( );
            detect_count     = 0;
        }
